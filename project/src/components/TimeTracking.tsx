...
import { Clock, Play, Square, MessageSquare, AlertCircle } from 'lucide-react';
...
export function TimeTracking() {
  ...
  const calculateWorkedHours = () => {
    if (!todayEntry?.clock_in || !todayEntry?.clock_out) return 0;
    
    const clockIn = new Date(todayEntry.clock_in);
    const clockOut = new Date(todayEntry.clock_out);
    return ((clockOut.getTime() - clockIn.getTime()) / (1000 * 60 * 60)).toFixed(1);
  };
  ...
  const calculateCurrentWorkedHours = () => {
    ...
    const clockIn = new Date(todayEntry.clock_in);
    return ((currentTime.getTime() - clockIn.getTime()) / (1000 * 60 * 60)).toFixed(1);
  };
  ...
  const getOvertimeMinutes = () => {
    if (!isAfterShiftHours()) return 0;
    const now = new Date();
    const shiftEnd = new Date();
    shiftEnd.setHours(15, 30, 0, 0);
    return Math.max(0, Math.floor((now.getTime() - shiftEnd.getTime()) / (1000 * 60)));
  };
                      ...
                      <div className="bg-orange-50 p-3 rounded-lg border border-orange-200">
                        <div className="flex items-center gap-2">
                          ...
                          <p className="text-sm font-medium text-orange-800">
                            Potential Overtime: {getOvertimeMinutes()} minutes
                          </p>
                        </div>
        ...
        <div className="mt-8 bg-gray-50 rounded-xl p-6">
          ...
          <div className="grid md:grid-cols-3 gap-4 text-sm">
            ...
            <div>
              ...
              <p className="font-semibold">After 4:00 PM (+35 PHP/hour)</p>
            </div>
            <div>
              <span className="text-gray-600">Undertime:</span>
              <p className="font-semibold">Before 3:30 PM (-25 PHP/hour)</p>
            </div>
          </div>
        </div>
      </div>
          ...
          <div className="bg-white rounded-2xl shadow-xl p-6 w-full max-w-md">
            ...
            <div className="bg-orange-50 p-4 rounded-lg mb-4">
              ...
              <p className="text-sm text-orange-700">
                Current overtime: {getOvertimeMinutes()} minutes past 3:30 PM
              </p>
            </div>
